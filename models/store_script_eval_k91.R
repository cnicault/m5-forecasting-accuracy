#
# Title  : M5 competition - Weekly forecast for one store
...
# parameters
MAX_LAGS <- 1940 # number of lags for test set
LAST_D <- 1941 # last training day
HORIZON <- 28 # forecast horizon
valid_date <- ymd("2016-05-23")
test_date <- ymd("2016-04-25")
KAGGLE <- 0
...
param <- args[[3]]
directory <- args[[4]]
performance <- list(store = store_id)
...
if(KAGGLE){
  calendar <- read_csv("../input/m5-forecasting-accuracy/calendar.csv")
  prices <- read_csv("../input/m5-forecasting-accuracy/sell_prices.csv")
}else{
  calendar <- read_csv(here::here("data", "calendar.csv"))
  prices <- read_csv(here::here("data", "sell_prices.csv"))  
}
...
save_df <- function(){
  if(KAGGLE){
    ...
    saveRDS(sales, here::here("wrk", "tmp", "sales.RDS"))
    saveRDS(sales_index, here::here("wrk", "tmp", "index.RDS"))
  }    
}
...
load_df <- function(){
  if(KAGGLE){
    ...
    sales <<- readRDS(here::here("wrk", "tmp", "sales.RDS"))
    sales_index <<- readRDS(here::here("wrk", "tmp", "index.RDS"))
  }    
}
...
# either for train or for test purpose.
create_long <- function(is_train = TRUE, store_id){
  
  if(KAGGLE){
    sales <- fread("../input/m5-forecasting-accuracy/sales_train_evaluation.csv", drop = paste0("d_", 1:(LAST_D - MAX_LAGS)))
  }else{
    sales <- fread(here::here("data", "sales_train_evaluation.csv"), drop = paste0("d_", 1:(LAST_D - MAX_LAGS)))
  }
...
lag_features<- function(){
  ...
  sales_base <- sales_base %>% 
    ...
    mutate(
      ...
      item_max7_lag28 = roll_maxr(value_lag28, 7),     # NEW
      item_max28_lag28 = roll_maxr(value_lag28, 28),   # NEW  
      item_min7_lag28 = roll_minr(value_lag28, 7),     # NEW
      item_min28_lag28 = roll_minr(value_lag28, 28),   # NEW  
...
extra_data <- function(){
  ...
  if(KAGGLE){
    load("../input/extra-data-full-cat/extra_data_eval.RData")
  }else{
    load(file = here::here("wrk", "extra_data_eval.RData"))  
  }
...
train_test_model <- function(){
  ...
  test_rmse <- RMSE(oof_pred, test_response[which(!is.na(test_response))])
  cat("best iter :" , lgb$best_iter, "best score :",  test_rmse,"\n" )
  iter <- lgb$best_iter
  
  performance[["test"]] <<- test_rmse
...
predict_test <- function(lgb, iter, sales_index, store_id){
  ...
  dates <- seq(test_date, test_date+27, by = "days")
  ...
  if(!KAGGLE){
    ...
    write_csv(features, here::here("output", directory, paste0("features_",store_id,".csv")))
  }
  ...
  valid_rmse <- RMSE(pred$value, test$value) 
  cat("best iter :" , lgb$best_iter, "best score :", valid_rmse,"\n" )
  
  performance[["valid"]] <<- valid_rmse
  ...
  if(KAGGLE){
    ...
    write_csv(predictions, here::here("output", directory, paste0("M5_store_test_",store_id,".csv")))  
  }
...
predict_final <- function(lgb, sales_index, store_id){
  ...
  # if(!KAGGLE){
  #   
  #   eval <- fread(here::here("data", "sales_train_evaluation.csv"), drop = paste0("d_", 1:LAST_D))
  #   
  #   eval <- eval %>%
  ...
  #   
  #   eval_rmse <- RMSE(pred$real, pred$pred) 
  #   cat("best score evaluation :", eval_rmse,"\n" )
  #   
  #   performance[["eval"]] <<- eval_rmse
  # }
  ...
  if(KAGGLE){
    ...
    write_csv(predictions, here::here("output", directory, paste0("M5_store_valid_",store_id,".csv")))  
  }
...
if(KAGGLE){
  lgb.save(lgb, "model.txt")
}else{
  lgb.save(lgb, file = here::here("wrk","model.txt"))
}
rm(lgb);rm(model_fit);free_mem()
mem_used()
load_df()
print("Loading model from disk")
if(KAGGLE){
  lgb <- lgb.load("model.txt")
}else{
  lgb <- lgb.load(here::here("wrk","model.txt"))
}
result <- predict_test(lgb, iter, sales_index, store_id)
rm(result); free_mem()
load_df()

### Comment this section to use the model trained on the test period (limit overfitting)
###         train            test  
### |---------------------|------|
###       train                    eval
### |---------------------|      |------| 
### Uncomment this section to retrain the model including the test period for final prediction on eval data
###         train            test  
### |---------------------|------|
###         train                  eval
### |----------------------------|------| 

# model_fit <- train_model(iter)
# lgb <- model_fit[[1]]
# print("Saving model to disk")
# if(KAGGLE){
#   lgb.save(lgb, "model.txt")
...
result <- predict_final(lgb, sales_index, store_id)
rm(result, sales, sales_index, lgb);free_mem()

write_csv(as_tibble(performance), here::here("output", directory, paste0("performance_", store_id,".csv")))
