...
create_long <- function(is_train = TRUE, store_id){
  ...
  if(KAGGLE){
    sales <- fread("../input/m5-forecasting-accuracy/sales_train_validation.csv", drop = paste0("d_", 1:(LAST_D - MAX_LAGS)))
  }else{
    sales <- fread(here::here("data", "sales_train_validation.csv"), drop = paste0("d_", 1:(LAST_D - MAX_LAGS)))
  }
...
extra_data <- function(){
  ...
  if(KAGGLE){
    load("../input/extra-data-full-cat/extra_data_valid.RData")
  }else{
    load(file = here::here("wrk", "extra_data_valid.RData"))  
  }
...
###       train                    valid
### |---------------------|      |------| 
### Uncomment this section to retrain the model including the test period for final prediction on valid data
###         train            test  
### |---------------------|------|
###         train                  valid
### |----------------------------|------| 
